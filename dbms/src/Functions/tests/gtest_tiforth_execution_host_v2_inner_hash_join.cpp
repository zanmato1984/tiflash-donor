...
public:
    void initializeContext() override
    {
        ...
        context.addMockTable(
            "tiforth_host_v2",
            "build_outer_null_build_input",
            {{"join_key", TiDB::TP::TypeLongLong}, {"build_payload", TiDB::TP::TypeLongLong}},
            {toNullableVec<Int64>("join_key", {1, {}, 5, 7}),
             toNullableVec<Int64>("build_payload", {10, 11, 50, 70})});

        context.addMockTable(
            "tiforth_host_v2",
            "build_outer_null_probe_input",
            {{"join_key", TiDB::TP::TypeLongLong}, {"probe_payload", TiDB::TP::TypeLongLong}},
            {toNullableVec<Int64>("join_key", {1, 5}),
             toNullableVec<Int64>("probe_payload", {100, 500})});
        ...
        context.addMockTable(
            "tiforth_host_v2",
            "build_outer_null_probe_key_probe_input",
            {{"join_key", TiDB::TP::TypeLongLong}, {"probe_payload", TiDB::TP::TypeLongLong}},
            {toNullableVec<Int64>("join_key", {1, {}, 5}),
             toNullableVec<Int64>("probe_payload", {100, 300, 500})});
        ...
        context.addMockTable(
            "tiforth_host_v2",
            "probe_outer_null_build_input",
            {{"join_key", TiDB::TP::TypeLongLong}, {"build_payload", TiDB::TP::TypeLongLong}},
            {toNullableVec<Int64>("join_key", {1, 1, {}, 5}),
             toNullableVec<Int64>("build_payload", {10, 11, 12, 50})});
    ...
    DonorRunResult runDonorNativeBuildOuterJoinWithEmptyBuild(size_t concurrency)
    {
        getDAGContext().clearWarnings();
        auto request = context.scan("tiforth_host_v2", "build_outer_probe_input")
                           .join(
                               context.scan("tiforth_host_v2", "build_outer_empty_build_input"),
                               tipb::JoinType::TypeRightOuterJoin,
                               {col("join_key")})
                           .project({"build_payload", "probe_payload"})
                           .build(context);
    ...
    DonorRunResult runDonorNativeBuildOuterJoinWithNullBuildKey(size_t concurrency)
    {
        getDAGContext().clearWarnings();
        auto request = context.scan("tiforth_host_v2", "build_outer_null_probe_input")
                           .join(
                               context.scan("tiforth_host_v2", "build_outer_null_build_input"),
                               tipb::JoinType::TypeRightOuterJoin,
                               {col("join_key")})
                           .project({"build_payload", "probe_payload"})
                           .build(context);
    ...
    DonorRunResult runDonorNativeBuildOuterJoinWithNullProbeKey(size_t concurrency)
    {
        getDAGContext().clearWarnings();
        auto request = context.scan("tiforth_host_v2", "build_outer_null_probe_key_probe_input")
                           .join(
                               context.scan("tiforth_host_v2", "build_outer_build_input"),
                               tipb::JoinType::TypeRightOuterJoin,
                               {col("join_key")})
    ...
    DonorRunResult runDonorNativeBuildOuterJoinWithNullBuildAndProbeKey(size_t concurrency)
    {
        getDAGContext().clearWarnings();
        auto request = context.scan("tiforth_host_v2", "build_outer_null_probe_key_probe_input")
                           .join(
    ...
    DonorRunResult runDonorNativeProbeOuterJoinWithEmptyProbe(size_t concurrency)
    {
        getDAGContext().clearWarnings();
        auto request = context.scan("tiforth_host_v2", "probe_outer_empty_probe_input")
                           .join(
                               context.scan("tiforth_host_v2", "probe_outer_build_input"),
                               tipb::JoinType::TypeLeftOuterJoin,
                               {col("join_key")})
    ...
    DonorRunResult runDonorNativeProbeOuterJoinWithNullBuildKey(size_t concurrency)
    {
        getDAGContext().clearWarnings();
        auto request = context.scan("tiforth_host_v2", "probe_outer_probe_input")
                           .join(
    ...
    DonorRunResult runDonorNativeProbeOuterJoinWithNullProbeKey(size_t concurrency)
    {
        getDAGContext().clearWarnings();
...
TEST_F(TestTiforthExecutionHostV2InnerHashJoin, InnerHashJoinPayloadParityLegacyEndSplitOutput)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    std::cout << "[tiforth-host-v2-inner-join-legacy-end] serial=1 warnings=" << adapter_serial.warning_count
              << " rows=" << adapter_serial.rows.size() << " parallel=2 warnings=" << adapter_parallel.warning_count
              << " rows=" << adapter_parallel.rows.size() << " donor_warnings=" << donor_serial.warning_count
...
TEST_F(TestTiforthExecutionHostV2InnerHashJoin, InnerHashJoinPayloadParityLegacyEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    std::cout << "[tiforth-host-v2-inner-join-legacy-end-partitions] serial=8 warnings="
...
TEST_F(
    ...
    BuildOuterHashJoinPayloadParityLegacyBuildEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    std::cout << "[tiforth-host-v2-build-outer-join-legacy-end-partitions] serial=8 warnings="
              << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    BuildOuterHashJoinPayloadParityNullBuildKeyLegacyBuildEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    auto donor_serial = runDonorNativeBuildOuterJoinWithNullBuildKey(1);
    auto donor_parallel = runDonorNativeBuildOuterJoinWithNullBuildKey(4);
    ...
    std::cout << "[tiforth-host-v2-build-outer-join-null-build-key-legacy-end-partitions] serial=8 warnings="
              << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    BuildOuterHashJoinPayloadParityNullProbeKeyLegacyBuildEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    auto donor_serial = runDonorNativeBuildOuterJoinWithNullProbeKey(1);
    auto donor_parallel = runDonorNativeBuildOuterJoinWithNullProbeKey(4);
    ...
    std::cout << "[tiforth-host-v2-build-outer-join-null-probe-key-legacy-end-partitions] serial=8 warnings="
              << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    BuildOuterHashJoinPayloadParityNullBuildAndProbeKeyLegacyBuildEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    auto donor_serial = runDonorNativeBuildOuterJoinWithNullBuildAndProbeKey(1);
    auto donor_parallel = runDonorNativeBuildOuterJoinWithNullBuildAndProbeKey(4);
    ...
    std::cout << "[tiforth-host-v2-build-outer-join-null-build-and-probe-key-legacy-end-partitions] serial=8"
              << " warnings=" << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
              << " donor_warnings=" << donor_serial.warning_count << " donor_rows=" << donor_serial.rows.size()
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    BuildOuterHashJoinPayloadParityEmptyBuildLegacyBuildAndProbeEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    auto donor_serial = runDonorNativeBuildOuterJoinWithEmptyBuild(1);
    auto donor_parallel = runDonorNativeBuildOuterJoinWithEmptyBuild(4);
    ...
    std::cout << "[tiforth-host-v2-build-outer-join-empty-build-legacy-end-partitions] serial=8 warnings="
              << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
...
TEST_F(TestTiforthExecutionHostV2InnerHashJoin, ProbeOuterHashJoinPayloadParityLegacyProbeEndSplitOutput)
{
    ...
    std::cout << "[tiforth-host-v2-probe-outer-join-legacy-end] serial=1 warnings="
              << adapter_serial.warning_count << " rows=" << adapter_serial.rows.size()
              << " parallel=2 warnings=" << adapter_parallel.warning_count
              << " rows=" << adapter_parallel.rows.size() << " donor_warnings=" << donor_serial.warning_count
              << " donor_rows=" << donor_serial.rows.size()
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    ProbeOuterHashJoinPayloadParityLegacyProbeEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    std::cout << "[tiforth-host-v2-probe-outer-join-legacy-end-partitions] serial=8 warnings="
              << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    ProbeOuterHashJoinPayloadParityEmptyProbeLegacyBuildAndProbeEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    auto donor_serial = runDonorNativeProbeOuterJoinWithEmptyProbe(1);
    auto donor_parallel = runDonorNativeProbeOuterJoinWithEmptyProbe(4);
    ...
    std::cout << "[tiforth-host-v2-probe-outer-join-empty-probe-legacy-end-partitions] serial=8 warnings="
              << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    ProbeOuterHashJoinPayloadParityNullBuildKeyLegacyProbeEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    auto donor_serial = runDonorNativeProbeOuterJoinWithNullBuildKey(1);
    auto donor_parallel = runDonorNativeProbeOuterJoinWithNullBuildKey(4);
...
TEST_F(
    TestTiforthExecutionHostV2InnerHashJoin,
    ProbeOuterHashJoinPayloadParityNullProbeKeyLegacyProbeEndSplitOutputHighPartitionRetainable)
{
    const bool strict_runtime_execution = Tiforth::requiresStrictRuntimeExecution();
    String load_error;
    auto maybe_api = Tiforth::loadExecutionHostV2Api(load_error);
    ...
    auto donor_serial = runDonorNativeProbeOuterJoinWithNullProbeKey(1);
    auto donor_parallel = runDonorNativeProbeOuterJoinWithNullProbeKey(4);
    ...
    std::cout << "[tiforth-host-v2-probe-outer-join-null-probe-key-legacy-end-partitions] serial=8 warnings="
              << adapter_serial_many_partitions.warning_count
              << " rows=" << adapter_serial_many_partitions.rows.size() << " parallel=8 warnings="
              << adapter_parallel_many_partitions.warning_count
              << " rows=" << adapter_parallel_many_partitions.rows.size()
